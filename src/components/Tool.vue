<template>
  <div id="tool">
    ...
    <el-button size="mini" @click="exportResult">
      导出中奖信息
    </el-button>
    ...
    >
      <el-form ref="form" :model="form" label-width="80px" size="mini">
        <el-form-item label="抽取奖项">
          <el-select v-model="form.category" placeholder="请选取本次抽取的奖项" style="width: 260px">
            <el-option
              :label="item.label"
              :value="item.value"
        ...
        <el-form-item>
          <el-button type="primary" @click="onSubmit" style="width: 100px;height: 100px;border-radius: 50%;margin-left: 50px;margin-top: 10px">立即抽奖</el-button>
...
import toExcel from '@/helper/json2excel'
...
export default {
  ...
  methods: {
    ...
    exportResult() {
      const originResult = getData('result');
      const originList = getData('list');
      const originLottery = getData('newLottery');

      for(let key in originResult){
        const personList = [];
        if(originResult[key] && originResult[key].length!=0){
            originResult[key].forEach(item=>{
              originList.forEach(list=>{
                if(item === list.key){
                  personList.push(list.name);
                }
              })
              originResult[key] = personList;
            })
        }
      }

      const reulstKeys = Object.keys(originResult);
      originLottery.forEach(item=>{
        if(reulstKeys.includes(item.key)){
            item.person = originResult[item.key];
        }
      });
      
      const newResult = [];
      originLottery.map(item=>{
      ...
      const th = ['花名', '奖品']
      const filterVal = ['name', 'priz'];
      const excelData = newResult;
      const data = excelData.map(v => filterVal.map(k => v[k]))
      const [fileName, fileType, sheetName] = ['年会中奖', 'xlsx', '中奖列表页']
      toExcel({th, data, fileName, fileType, sheetName});
      console.log('resultPrize', originLottery);
    },
  }
};
</script>
